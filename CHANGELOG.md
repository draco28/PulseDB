...
- **`ApplyResult` gains `failed`** (`pulsedb::sync::applier::ApplyResult`, reachable through the public `sync::applier` module). Struct literals without `..Default::default()` must add the new field. It counts only the applier's error arm, unlike `skipped`, which counts idempotent no-ops *and* failures and is unchanged — `SyncStats` and every existing field keep their meaning. (`sync` feature.)
