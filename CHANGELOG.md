...
- **Cross-provider-mismatch detection (common case).** Re-opening an existing manifold via `open_with_embedder` with an embedder whose persisted identity differs from the stored one is refused with a typed `PulseDBError::EmbeddingProviderMismatch`. This **detects the common case** of reopening with a different injected embedder; it is not a complete prevention mechanism — see Known Limitations for the documented bypasses.
...
- **The mismatch guard has known bypasses (cross-provider mixing is *detected*, not *prevented*).** Three scenarios can still mix vectors from different providers into one HNSW index: (a) **the `open` path skips the guard entirely** — a store stamped for provider A can be reopened via `PulseDB::open` (builtin B), which inserts B's vectors while `provider_identity()` reports A (`pulsedb-internal` #7); (b) **`embedding: Some(vector)` is always accepted if the dimension matches** — per-record vectors from arbitrary models can be inserted under a stamped provider (`pulsedb-internal` #8); (c) **`OnnxEmbedding`'s default `model_id` (`"main_graph"`) is not unique** — unrelated ONNX models can collide on `{builtin-onnx, main_graph}` and pass the guard (`pulsedb-internal` #9). In practice these are bounded if a consumer uses `open_with_embedder` exclusively and writes only via the injected embedder (`embedding: None`). Tracked as follow-ups (#7–#12).
- **Concurrent first-open is a check-then-set race** (`pulsedb-internal` #11). Two processes opening the same unstamped store simultaneously with different providers can both observe no stamp and commit competing stamps. Bounded in single-process use.
- **A refused open may have already mutated storage** (`pulsedb-internal` #12). The mismatch check runs *after* `open_parts` (storage open, possible schema migration, index load), so a refused reopen can leave the store with non-stamp mutations. The narrower true claim is "does not overwrite the provider stamp," not "writes nothing."
