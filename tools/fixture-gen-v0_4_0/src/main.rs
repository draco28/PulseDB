...
//! the published 0.4.0 public API (default features — no ONNX; embeddings are
...
//!
//! Usage: `fixture-gen-v0_4_0 <out.redb> <out.content.json>`
//!
//! v0.4.0 is schema-v2, so this fixture exercises ALL THREE migration axes:
//! axis-1 (redb file-format v2→v3), axis-2 (bincode→postcard value re-encode),
//! and axis-3 (the v2→v3 logical reshape: migrate_experiences_v2_to_v3 +
//! migrate_wal_v1_to_v2). v0.4.0 PREDATES the decay surface (no DecayConfig).

use pulsedb::{
    AgentId, Config, EmbeddingDimension, ExperienceType, InsightType, NewDerivedInsight,
    NewExperience, NewExperienceRelation, PulseDB, RelationType, Severity,
};
use redb::{MultimapTableDefinition, ReadableMultimapTable, ReadableTable, TableDefinition};
use serde_json::{json, Value};

// Copy-through table definitions, mirrored VERBATIM from pulsehive-db 0.4.0
// `src/storage/schema.rs` (identical layout to 0.5.1 for these tables).
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let redb_path = args
        .next()
        .ok_or("usage: fixture-gen-v0_4_0 <out.redb> <out.content.json>")?;
    ...
    // ---- WRITE PHASE (published 0.4.0 public API; no decay surface) ----
    let cfg = Config::with_external_embeddings(EmbeddingDimension::D384);
    ...
    let content = json!({
        "fixture_blob": std::path::Path::new(&redb_path)
            .file_name().and_then(|s| s.to_str()).unwrap_or("real-v0.4.0.redb"),
        "source_release": "0.4.0",
        "generated_by": "fixture-gen-v0_4_0",
        "real_published_crate": "pulsehive-db (=0.4.0, crates.io)",
        "schema_version": schema_version,
        "embedding_dimension": embedding_dimension,
        "instance_id": if instance_id.is_empty() { Value::Null } else { Value::String(instance_id.clone()) },
        "instance_id_note": if instance_id.is_empty() {
            "ABSENT in this schema-v2 store: v0.4.0 persists instance_id ONLY behind the `sync` feature (default-features stores have none). A fresh instance_id is MINTED during the v2→v3 migration — so 4.02 must assert an instance_id now EXISTS (minted), not a specific value. Contrast the v0.5.1 fixture, which already carries an instance_id that migration must PRESERVE."
        } else {
            "present in the pre-migration store"
        },
        "migration_axes": [
            "axis1_redb_file_format_v2_to_v3",
            "axis2_bincode_to_postcard_value_reencode",
            "axis3_logical_schema_v2_to_v3_reshape"
        ],
        "migration_axes_note": "v0.4.0 is schema-v2, so this fixture exercises ALL THREE migration axes, including axis-3 (migrate_experiences_v2_to_v3 + migrate_wal_v1_to_v2).",
        "feature_set": {
        ...
        "decay_config": {
            "present": false,
            "note": "v0.4.0 PREDATES the decay surface — there is no DecayConfig / decay field in the 0.4.0 public API. Recorded here as an explicit absence."
        },
        "expected_search": {
        ...
        "coverage": {
            "included": [
                ...
                "watch_events", "db_metadata"
            ],
            "decay_absent": "v0.4.0 predates the decay surface (no DecayConfig) — documented absence, not a gap.",
            ...
            "residual_gap_v0_3_0_wal_v1": "v0.3.0 / WAL-v1 (the logical schema BEFORE v2) is NOT synthesized here; this is the documented residual gap — v0.4.0 is schema-v2 (WAL-v2), so a genuine WAL-v1 / v0.3.0 artifact is out of scope and left as a known residual, not faked."
        },
        "falsification_contract": "A corrupted or truncated real-v0.4.0.redb MUST fail loudly downstream: this fixture's blob_sha256 (4.01 provenance AC) + 4.02's corrupt-fixture negative test together guarantee a mutated/truncated blob fails rather than silently passing."
    });

    std::fs::write(&content_path, serde_json::to_vec_pretty(&content)?)?;
    eprintln!(
        "fixture-gen-v0_4_0: wrote {} (schema={}, {} experiences, {} embeddings raw) + {}",
        redb_path,
        schema_version,
        recorded.len(),
        emb_raw.len(),
